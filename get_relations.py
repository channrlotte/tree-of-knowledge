...
def is_russian(text: str) -> bool:
    ...
    if not text.strip() or text[0] == "-" or text[-1] == "-":
        return False
...
def head_in_named_entity(
    doc: Doc, span: Span
) -> Tuple[Optional[Token], Dict[Token, Token]]:
    """
...
def normalize_named_entity(
    doc: Doc, span: Span, head: Token, tokens_heads: Dict[Token, Token]
) -> str:
    """
    ...
    for token, token_head in tokens_heads.items():
        ...
        if (
            token_pos in ("ADJF", "PRTF")
            or any(tag in token_tag for tag in ("Surn", "Patr"))
        ) and token_head == head:
    ...
    for ent in doc.ents:
        if is_russian(ent.text):
            ...
            if head:
                ...
                chunks[head] = [
                    chars,
                    normalize_named_entity(doc, ent, head, tokens_heads),
                ]
...
def extract_noun_chunks(doc: Doc, chunks: Chunks) -> Chunks:
    ...
    for token in doc:
        if (
            is_russian(token.text)
            and token.pos_ in ("NOUN", "PROPN")
            and token not in chunks
        ):
...
def solve_anaphora(doc: Doc, chunks: Chunks) -> Chunks:
    ...
    for token in doc:
        if is_russian(token.text) and token.pos_ == "PRON":
            ...
            if (
                not any(
                    token.lemma_.startswith(pron) or token.lemma_.endswith(pron)
                    for pron in ("что", "то", "либо", "нибудь")
                )
                and not token.text.lower().startswith("вс")
                and morph.get("Person", "Third") == "Third"
            ):
                ...
                for chunk_key, chunk_value in chunks.items():
                    if latest_end <= chunk_value[0][0] < token.idx:
                        ...
                        if all(
                            chunk_morph.get(k, None) == morph[k]
                            for k in morph
                            if k != "Person"
                        ):
...
def collect_preds(
    subj: Token, pred: Token, subjs_preds: Dict[Token, Any], visited: Set[Token] = None
) -> None:
    """
    ...
    for chunk_key, chunk_value in chunks.items():
        if chunk_key.dep_ in ("nsubj", "nsubj:pass"):
            ...
            if conj and conj_child in chunks:
                subj = [chunk_value[1], conj.lemma_, chunks[conj_child][1]]
            else:
                subj = chunk_value[1]
...
def build_relations(chunks: Chunks, subjs_preds: Dict[Token, Any]) -> List[List]:
    """
    ...
    Returns:
        List[List]: Список отношений между концепциями.
    """

    relations = []

    for chunk_key, chunk_value in chunks.items():
        if chunk_key.head in subjs_preds and chunk_key.dep_ not in (
            "nsubj",
            "nsubj:pass",
        ):
            conj_child = check_dep(chunk_key, ["conj"])
            ...
            if conj and conj_child in chunks:
                relation_type = 4 if type(subjs_preds[chunk_key.head]) == list else 3
                relations.append(
                    [
                        relation_type,
                        subjs_preds[chunk_key.head],
                        chunk_key.head.lemma_,
                        [chunk_value[1], conj.lemma_, chunks[conj_child][1]],
                    ]
                )

            else:
                relation_type = 2 if type(subjs_preds[chunk_key.head]) == list else 1
                relations.append(
                    [
                        relation_type,
                        subjs_preds[chunk_key.head],
            ...
            if conj and conj_child in chunks:
                ...
                relations.append([1, chunks[chunk_key.head][1], adp, chunk_value[1]])

    return relations


def get_relations(text: str) -> List[List]:
    doc = nlp(text)
...
if __name__ == "__main__":
    ...
    text = """У каждого человека есть мама, которая любит его несмотря ни на что.
    Она всегда готова прийти на помощь и выручить в трудную минуту, зачастую жертвуя своим здоровьем.
    Сын или дочь может рассказать маме все свои тайны и секреты, не дающие покоя днем и ночью, и услышать слова поддержки.
    """

    with open("data/relations/mother/mother.json", "wt", encoding="utf-8") as f:
        json.dump(get_relations(text), f, ensure_ascii=False)
